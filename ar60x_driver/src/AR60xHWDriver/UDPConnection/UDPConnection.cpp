...
void UDPConnection::ConnectToHost(string robot_address, uint16_t  local_port, uint16_t robot_port)
{
    if (!is_connected_)
    {
...
{
    lock_guard<mutex> guard(send_locker_);
...
{
    lock_guard<mutex> guard(recv_locker_);
...
future<UDPConnection::SocketResult>  UDPConnection::receive(ip::udp::socket& socket, uint8_t *buf, size_t buffer_size)
{
    auto promise = make_shared<std::promise<SocketResult>>();
...
future<UDPConnection::SocketResult>  UDPConnection::send(ip::udp::socket& socket, uint8_t *buf, size_t buffer_size)
{
    auto promise = make_shared<std::promise<SocketResult>>();
