...
robot_msgs::TypeJointMode AR60xRecvPacket::JointGetMode(const JointData &joint) const
{
    robot_msgs::TypeJointMode state;

    uint8_t value = byte_array_[joint.channel * 16 + JointStateAddress];
    uint8_t status_mask = value & 0b11;

    if(status_mask == 0b00)
        state.mode = robot_msgs::TypeJointMode::BREAK;
    else if(status_mask == 0b01)
        state.mode = robot_msgs::TypeJointMode::STOP;
    else if(status_mask == 0b10)
        state.mode = robot_msgs::TypeJointMode::RELAX;
    else if(status_mask == 0b11)
        state.mode = robot_msgs::TypeJointMode::TRACE;
...
robot_msgs::TypePid AR60xRecvPacket::JointGetPidGains(const JointData &joint) const
{
    //FUCK Android Technology company
    robot_msgs::TypePid pid;
...
robot_msgs::TypeSupplyState AR60xRecvPacket::PowerGetJointSupplyState(const JointData &joint) const
{
    robot_msgs::TypeSupplyState state;
...
robot_msgs::TypeSupplyState AR60xRecvPacket::PowerGetSourceSupplyState(const PowerSources supply) const
{
    robot_msgs::TypeSupplyState state;
...
robot_msgs::FeetSensors AR60xRecvPacket::SensorsGetFeet() const
{
    robot_msgs::FeetSensors state;
...
robot_msgs::TypeFootSensor AR60xRecvPacket::sensorGetFoot(const uint8_t groupId) const
{
    robot_msgs::TypeFootSensor data;
